            ...
            if(!empty($_POST['last'])){
                if(ctype_alpha($_POST['last']) && strlen($_POST['last'])<=30){
                    ...
                    if(!empty($_POST['gender']) && in_array($_POST['gender'],$gender_arr)){
                        $gender=$_POST['gender'];
                        if(!empty($_POST['class']) && in_array($_POST['class'],$class_arr)){
                            $class=$_POST['class'];
                            if(!empty($_POST['section']) && in_array($_POST['section'],$section_arr)){
                                $section=$_POST['section'];
                                $sql="INSERT INTO `students`(`roll_no`, `firstname`, `lastname`, `gender`, `class`, `section`, `teacher`) VALUES ('$firstname','$lastname','$gender','$class','$section','$user-id')";
                                ...
                                if($query){
                                    ...
                                    <script>
                                    alert("Student add");
                                    window.loation.href="main.php";</script>';
                                }
                            }
                            else{
                                echo '
                        ...
                        echo '
                    <script>
                        alert("Gender Name Should Not Be Empty");
                    </script>
                    ';
                    }
                }
...
<table>
    ...
    <tr>
        ...
        <td><label for=""><input type="radio" name="gender">Male</label>
        <label for=""><input type="radio" name="gender" id="">Female</label></td>
    </tr>
    <tr>
        <td><label for="">Class :</label></td>
        <td>
            <select class="class"><option value="Choose">Choose</option>
