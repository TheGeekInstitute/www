...
if(isset($_FILES['photo']) && $_FILES['photo']['size']>0){
    ...
    $ext_lst=["jpg","jpeg","png","webp","avif","gif"];
    $dest="uploads/".bin2hex(random_bytes(10)).".".$ext;
    if(in_array($ext,$ext_lst)){
        if($size<=2){
            if(move_uploaded_file($tmp,$dest)){
                $sql="INSERT INTO `form`(`studentname`, `fathername`, `mothername`, `dob`, `gender`, `present_division`, `present_district`, `present_address`, `permanent_division`, `permanent_district`, `permanent_address`, `religion`, `nationality`, `phone`, `email`, `nid_number`, `blood_group`, `occupation`, `photo`) VALUES ('$sname','$fname','$mname','$dob','$gender','$present_division','$present_district','$present_address','$permanent_division','$permanent_district','$permanent_address','$religion','$nationality','$phone','$email','$nid_phone','$blood_group','$occupation','$dest')";
               ...
               if($query){
                ...
                $mes="Can not Upload Image at This Time";
                $error=true;
            }
        }
        else{
            $mes="Image Should not be grater than 2MB";
            $error=true;
        }
    }
    else{
        $mes="Please Choose a valid Image";
        $error=true;
    }

        }
        else{
            $mes="Attach your Photo";
            $error=true;
        }
    }
    else{
